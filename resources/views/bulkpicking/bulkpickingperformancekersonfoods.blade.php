...
'left=20,top=20,width=1000,height=1000,toolbar=1,resizable=0'); return false;" >Advance</a>
    <table class="table" id="livepickingtable">
        ...
        <th style="color:#61ff13;font-size: 25px;font-family: sans-serif;">Created</th>
        <th style="color:#61ff13;font-size: 25px;font-family: sans-serif;">Add-On Time</th>
		<th style="color:#61ff13;font-size: 25px;font-family: sans-serif;">GW1</th>
		<th style="color:#61ff13;font-size: 25px;font-family: sans-serif;">GW2</th>
        <th style="color:#61ff13;font-size: 25px;font-family: sans-serif;">PER</th>
        <th style="color:#61ff13;font-size: 25px;font-family: sans-serif;">NF</th>
        ...
        @foreach($performance as $val)
            ...
            @if( $val->blnAttended =="DONE")
                ...
                <td>{{$val->Route}}</td>
                <td>{{$val->dDelDate}}</td>
                <td>{{$val->dtCreation}}</td>
                <td>{{$val->dtAddOnTime}}</td>
                <td>{{$val->GW1}}</td>
                <td>{{$val->GW2}}</td>
                <td>{{$val->Per}}</td>
                <td>{{$val->nf}}</td>
