        ...
        <div class="col-lg-12">
            ...
            <div class="col-lg-12" style="">
                <table class="table2 table-bordered " id="createdOrders" style="overflow-y: auto;width:100%;color: black;    font-weight: 700;" tabindex=0>
                    <thead>
                    <tr>
                        <th class="col-sm-1">OrderId</th><th class="col-sm-1">Invoice no</th><th class="col-sm-1">Cust Code</th><th>Cust Name</th><th class="col-sm-1">>Order Types</th><th class="col-sm-1">>Route</th><th class="col-sm-1">Delivery Date</th><th>Order date</th><th>Reference No</th><th>Created By</th><th>Total Inv</th></tr>
                    </thead>
                    <tfoot>
                    <tr >
                        <th >OrderId</th><th >Invoice no</th><th >Cust Code</th><th>Cust Name</th><th>Order Types</th><th>Route</th><th>Delivery Date</th><th>Order Date</th><th>Reference No</th><th>Created By</th><th>Total Inc</th></tr>
                    </tfoot>
                ...
                <div class="form-group  col-md-12" >
                    ...
                    <div  >
                        <fieldset>
                            ...
                            <button type="button" id="treattheauthaccountasquotation" class="btn-danger btn-xs pull-right" style="margin-top: 29px;margin-right: 15px;">Notify Managers And Continue</button>
                        </fieldset>
                    </div>
                </div>
            </form>
                                        ...
                                        success: function (dataDetails) {
                                            ...
                                            $.each(dataDetails, function (keyDetails, valueDetails) {
                                                ...
                                                var $row = $('<tr id="new_row_ajax'+tokenId+'" class="fast_remove" style="font-weight: 600;font-size: 11px;">' +
                                                    ...
                                                    '<td contenteditable="false"  class="col-md-1"><input type="text" name="instockReadOnly" id ="instockReadOnly_' + tokenId + '" value="' + valueDetails.QtyInStock + '"  class="instockReadOnly_ resize-input-inside inputs" style="font-weight: 800;width: 80%;color:blue;"><select name="col2" id ="col2'+tokenId+'" class="col2 '+classAnonymouscols+'"></select>' +
...
//View PDF
            ...
            $(document).ready(function() {
                $('#table,#tableDispatch tbody').on('focusout','tr', function () {
                    ...
                    if (fieldQuantity.length > 0 && isAuthMyLine !='authorised' ){
                        $.ajax({
                            url: '{!!url("/getAvailable")!!}',
                            type: "POST",
                            data: {
                                prodcode:theProductCode_

                            },
                            success: function (dataAvailable) {

                                console.debug("***********************dataAvailable" +dataAvailable+"--------------------------"+isAuthMyLine);

                                if ((parseFloat(dataAvailable) < parseFloat(fieldQuantity)) && (theProductCode_.length > 0 && $Description.length > 0) && isAuthMyLine !='authorised' && authString !='preauthorised' )
                                {

                                    $('#appendErrormsg').empty();
                                    $('#appendErrormsg').append("There is less available [ "+dataAvailable+" ]");
                                    commonDialog();
                                    $('#noThanksRedo').off().click(function(){
                                        if (theOrdersDetailsId.length > 0 )
                                        {

                                            var orderLineID = theOrdersDetailsId;
                                            $.ajax({
                                                url:'{!!url("/deleteOrderDetails")!!}',
                                                type: "POST",
                                                data: {
                                                ...
                                                success: function(data){
                                                    ...
                                                    {
                                                        if (($('#invoiceNo').val()).length < 1) {
                                                            $('#new_row_ajax'+$hiddenToken).remove();
                                                            $( "#authorisations" ).dialog('close');
                                                            calculator();
                                                            generateALine2();
                                                        }
                                        ...
                                        $.ajax({
                                            ...
                                            success: function(data){
                                                ...
                                                }else
                                                {
                                                    $('#userAuthName').val('');
                                                    $('#userAuthPassWord').val('');
                                                    $('#title_'+$hiddenToken).val('preauthorised');
                                                    console.debug('///////////////////'+$('#title_'+$hiddenToken).val());

                                                    consoleManagementAuths('{!!url("/logMessageAuth")!!}',12,1,'Authorized less Available: '+dataAvailable+' VS Qty of:'+fieldQuantity+' by '+data[0].UserName,
                                                        0,$('#orderId').val(),'',$('#inputCustAcc').val(),0,0,0,$('#userNewVariable').val(),$('#orderId').val(),0,computerName,$('#orderId').val(),0,data[0].UserID,data[0].UserName);
                                                    $( "#authorisations" ).dialog('close');
                                                    calculator();
                                                }
