...
<h2>Customer Pricing [{{$custid}}] </h2>
<h5>PLEASE NOTE THAT THE INSTOCK AND AVAILABLE QUANTITY DATA IS NOT POPULATED ON DEMAND</h5>
...
<script type="text/javascript" charset="utf-8">
    // specify the columns
    var customerid = JSON.stringify({!! json_encode($custid) !!});
    customerid = JSON.parse(customerid);
    console.debug("*************"+customerid);
        var columnDefs = [
            {headerName: "Code", field: "PastelCode"},
        ...
        fetch('{!!url("/custometPricingJson")!!}/'+customerid).then(function(response) {
